...
var takeoffs = {
  ...
  backFullTwist: ['invert backside pop', 'invert backside punch'],
...
'hook', 'turn step', 'step behind', 'cheat', 'frontside pop', 'inside pop', 'swing', 'wrap' // "back swing",
// "front swing",
...
'hook carry-through', 'round carry-through', 'reverse pop', 'frontside pop', 'inside pop', 'backside pop', 'invert frontside pop', // for flips
...
var tricks = {
  ...
  // },
  {
    ...
    starter: true,
    setups: ['back swing', 'round carry-through', 'inside leg reversal', 'left right redirect', 'skip hook'],
...
function generateCombo() {
  ...
  var trick1 = generateFirstTrick(tricks[randomLevel]); // TODO: Add handle hook functionality here (perhaps no generateFirstTrick function?)
...
function handleTakeoff(transition, takeoff, trick) {
  ...
  if (takeoffModifiers.includes(takeoff) && takeoff !== 'hook') return formatMod(takeoff, trick.name); // If takeoff is not a takeoff mod or it is a hook, then return undefined
